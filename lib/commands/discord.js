...
self.greet = function (commandText, message, channel = null) {
  ...
  if (!message.mentions) return `You didn't tell me who to greet!`;
  const greetRegex = /([\w\d@\+_]{1,})\s?(.*)/i;
  const greetMatch = commandText.match(greetRegex);
  const greeting = greetMatch[2] ? greetMatch[2] : ":upside_down:";
  ...
  if (user) {
    ...
    if (member) {
      channel.send(`Welcome to ${channel}, ${user.toString()} ${greeting}!`);
      return false;
    }
  }
};
